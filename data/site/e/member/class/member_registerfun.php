...
function phoneRegister($add){
    ...
    //手机验证码
    $captcha = trim($add['captcha']);
    checkCapt($phone,$captcha,2);
    ...
    {
        $spacestyleid=ReturnGroupSpaceStyleid($groupid);
        $sql1=$empire->query("insert into {$dbtbpre}enewsmemberadd(userid,spacestyleid,regip,lasttime,lastip,loginnum,regipport,lastipport,phone) values('$userid','$spacestyleid','$regip','$lasttime','$regip','1','$regipport','$regipport','$phone');");
    }
    upCaptSta($phone,$captcha,2);
