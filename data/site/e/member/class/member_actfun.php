...
function SendGetPasswordPhone($add){
    global $empire,$dbtbpre,$public_r;
    if(!$public_r['opengetpass'])
    {
        printerror('CloseGetPassword','',1);
    ...
    //验证码
    checkCapt($username,$captcha,4);
    upCaptSta($username,$captcha,4);
    $username=RepPostVar($username);
    $captcha=addslashes(RepPostStr($captcha));
    $captcha=RepPostVar($captcha);
    ...
    $passr=QMReturnCheckPass($ur['userid'],$username);
    $authstr=$passr['dotime'].'||'.$passr['ecms'].'||'.$passr['rnd'];
    $sql=DoUpdateMemberAuthstr($ur['userid'],$authstr);
    $url=eReturnDomainSiteUrl().'e/member/GetPassword/getpass.php?id='.$ur['userid'].'&cc='.$passr[checkpass].'&tt='.$passr['dotime'];

    //发送邮件
    printerror("SendGetPasswordPhoneSucess",$url,1);
}
