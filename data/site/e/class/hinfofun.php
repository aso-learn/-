...
function EditNews($add,$userid,$username){
	...
	if(!empty($add[cardpass]))
	{
		...
		for($i=0;$i<$count;$i++)
		{
			$cardsql=$empire->query("insert into {$dbtbpre}ecms_card_list(fahaoid,cardpass,starttime,endtime,carddate,cardname,status) values('$add[id]','".addslashes(trim($f1[$i]))."','$starttime','$endtime',0,'',0);");
		}
		if($cardsql)
		{
			$total=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_card_list where fahaoid='$add[id]'");
			$num=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_card_list where fahaoid='$add[id]' and status=0");
			$update=$empire->query("update {$dbtbpre}ecms_cards set shuliang='$total',renum='$num' where id='$add[id]'");
		}
	}
	//投票
