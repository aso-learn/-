...
function page2($num,$line,$page_line,$start,$page,$search){
	...
	if($page<>0)
	{
		...
		$pagepr=$page-1;
		$prepage='<a href="'.$url.'='.$pagepr.$search.'" class="prev">'.$fun_r['adminpripage'].'</a>';
	}else{
        $prepage="<span class=\"prev empty\" title=\"上一页\">上一页</span>";
    }
	//下一页
	if($page!=$totalpage-1)
	{
		$pagenex=$page+1;
		$nextpage='&nbsp;<a href="'.$url.'='.$pagenex.$search.'"  class="next">'.$fun_r['adminnextpage'].'</a>';
		$lastpage='&nbsp;<a href="'.$url.'='.($totalpage-1).$search.'">'.$fun_r['adminlastpage'].'</a>';
	}else{
        $nextpage="<span class=\"next empty\" title=\"下一页\">下一页</span>";
    }
	...
	$returnstr=$prepage.'<span class="rest">共'.$totalpage.'页：'.$page.'/'.$totalpage.'</span>'.$nextpage;
...
'KeyId'=>'".addslashes($r[KeyId])."',
'KeySecret'=>'".addslashes($r[KeySecret])."',
'model'=>'".addslashes($r[model])."',
'SignName'=>'".addslashes($r[SignName])."',
