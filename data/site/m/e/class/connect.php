...
function page1($num,$line,$page_line,$start,$page,$search){
	...
	if($page<>0)
	{
		...
		$pagepr=$page-1;
		$prepage='<a href="'.$url.'='.$pagepr.$search.'" class="prev">'.$fun_r['pripage'].'</a>';
	}else{
	...
	{
		$pagenex=$page+1;
		$nextpage='&nbsp;<a href="'.$url.'='.$pagenex.$search.'" class="next">'.$fun_r['nextpage'].'</a>';
    ...
    $pageone=$page+1;
	$returnstr=$prepage.'<span class="rest">共'.$totalpage.'页：'.$pageone.'/'.$totalpage.'</span>'.$nextpage;
	return $returnstr;
}
