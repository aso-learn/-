...
//验证码校对
function checkCapt($str,$captcha,$type,$enews='phone'){
    global $empire,$dbtbpre;
    $time = time();
    if ($enews == 'phone'){
        $capt = $empire->fetch1("select captcha,capttime from {$dbtbpre}enewscapt where phone='$str' and `type` = '$type' and status = 1 order by capttime desc limit 1");
    }elseif($enews == 'email'){
        $capt = $empire->fetch1("select captcha,capttime from {$dbtbpre}enewscapt where email='$str' and `type` = '$type' and status = 1 order by capttime desc limit 1");
    }
    if ($captcha != $capt[captcha]){
        printerror("captError","history.go(-1)",1);
    }
    if ($time - $capt[capttime] >300){
        printerror("captZhanTime","history.go(-1)",1);
    }
    return true;
}

//验证码状态更换
function upCaptSta($phone,$captcha,$type,$enews='phone'){
    global $empire,$dbtbpre;
    if ($enews == 'phone'){
        $res = $empire->query("update {$dbtbpre}enewscapt set status=0 where phone='$phone' and captcha='$captcha' and `type`='$type' limit 1");
    }elseif($enews == 'email'){
        $res = $empire->query("update {$dbtbpre}enewscapt set status=0 where email='$phone' and captcha='$captcha' and `type`='$type' limit 1");
    }
    if (!$res){
        printerror("statusUpError","history.go(-1)",1);
    }
    return true;
}
?>